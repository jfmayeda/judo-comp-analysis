...
function TournamentPrintPack() {
  ...
  const loadAthletes = async () => {
    try {
      ...
      const athletesData = await Promise.all(
        athleteIds.map(id => getAthleteWithNotes(id))
      );
      setAthletes(athletesData.filter(a => a !== null) as AthleteWithNotes[]);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-gray-700">Loading tournament pack...</p>
      </div>
    );
  }
  ...
  if (athletes.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-gray-700">No athletes selected</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto p-8">
        ...
        {athletes.map((athlete, index) => (
          <div key={athlete.id} className={`print-page ${index < athletes.length - 1 ? 'mb-12' : ''}`}>
            {/* Header Section */}
...
export default function TournamentDayPrintPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center">
